...
int main(int argc, char * argv[])
{
	...
	for(int i=0;i<4;i++)
	{
		p_KeyClass[i]=(1+KeyClass[i]*(pow(2,-10)))/2.0;
	}
	...
	for(int test_num=0;test_num<TotalTestNum;test_num++)
	{
		...
		for(int i=0;i<4;i++)
		{
			tmpArr[i]=log(Pi[i])-0.5*log(p_KeyClass[i])-0.5*log(1-p_KeyClass[i])-((counter-pow(2,IterativeNum)*p_KeyClass[i])*(counter-pow(2,IterativeNum)*p_KeyClass[i])*(pow(2,128)-1))/(2*counter*p_KeyClass[i]*(1-p_KeyClass[i])*(pow(2,128)-pow(2,IterativeNum)));
		}
		for(int i=0;i<4;i++)
		{
			cout<<tmpArr[i]<<" ";
