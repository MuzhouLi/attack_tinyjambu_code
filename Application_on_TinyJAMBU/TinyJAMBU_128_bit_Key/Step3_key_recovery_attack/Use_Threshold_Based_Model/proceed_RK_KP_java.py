	...
	with concurrent.futures.ProcessPoolExecutor() as executor:
		...
		for N in range(len(Chosen_N)):
			for key_index in range(len(Chosen_KeyDiff)):
				cmd.append("java -cp .:commons-math3-3.6.1.jar ComputeCDFValues_RK "+str(Chosen_N[N])+" 0 "+str(Chosen_KeyDiff[key_index])+" > R_RK_KP_N"+str(Chosen_N[N])+"_"+str(Chosen_KeyDiff[key_index])+".txt")
