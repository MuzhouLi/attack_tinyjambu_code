	...
	with concurrent.futures.ProcessPoolExecutor() as executor:
		...
		for N in range(len(Chosen_N)):
			for ClassNum in range(727):#727
				cmd.append("java -cp .:commons-math3-3.6.1.jar MultiThreads_ComputeCDFValues_NRK_KP "+str(Chosen_N[N])+" "+str(ClassNum)+" > R_NRK_KP_N"+str(Chosen_N[N])+"_"+str(ClassNum)+".txt")
