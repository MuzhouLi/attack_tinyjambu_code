...
int main(int argc, char * argv[])
{
	...
	for(int test_num=0;test_num<TotalTestNum;test_num++)
	{
		...
		for(int i=0;i<3;i++)
		{
			tmpArr[i]=log(Pi_1[i])-pow(testCor-pow(2,IterativeNum-1-10)*KeyClass_1[i],2)/pow(2,IterativeNum);
		}
		maxV=tmpArr[0];
		determined_index_1=0;
		for(int i=1;i<3;i++)
		...
		if(theory_index_1==determined_index_1)
		{
			...
			for(int i=0;i<4;i++)
			{
				tmpArr[i]=log(Pi_2[i])-pow(testCor-pow(2,IterativeNum-1-10)*KeyClass_2[i],2)/pow(2,IterativeNum);
			}
			maxV=tmpArr[0];
			determined_index_2=0;
			for(int i=1;i<4;i++)
			...
			if(theory_index_2==determined_index_2)
			{
				...
				for(int i=0;i<3;i++)
				{
					tmpArr[i]=log(Pi_3[i])-pow(testCor-pow(2,IterativeNum-1-10)*KeyClass_3[i],2)/pow(2,IterativeNum);
				}
				maxV=tmpArr[0];
				determined_index_3=0;
				for(int i=1;i<3;i++)
