'''
This code is used to search all linear trails given specific linear hull for TinyJambu utilizing the simple model, which only counts number of ANDs and then we compute its correlation according to Song et al.'s work.
Besides, key bits involved in these trails are also drived.
All these information are stored in the file named "LinearTrails_RepeatedSolutionsRemoved.txt".
'''
...
if __name__ == "__main__":
	try:
		opts, args=getopt.getopt(sys.argv[1:],"r:k:")
		if len(opts)!=2:
			print('usage: python2 -u Step1_search_all_trails_in_linear_hull.py -r rounds -k 128 > LinearTrails.txt')
			sys.exit(2)
	...
	while(goon):
		...
		if nSol>0:
			...
			goon=True
			# Add constraints to remove the former solution and others belonging to the same equivalent class in the model
			constr=0
			for s in range(15):
				t_s=list_ts[s]
				for j in range(t_s+1):
